...
从 NMPA UDI 数据库下载日度增量数据。

用法:
    python udid_daq_daily.py              # 下载最新日度数据
    python udid_daq_daily.py -l           # 列出可用日期
    python udid_daq_daily.py -d 20260326  # 下载指定日期
    python udid_daq_daily.py -a           # 下载所有可用日期
    python udid_daq_daily.py --excel      # 输出 Excel 格式
    python udid_daq_daily.py --db oracle  # 写入数据库
...
def main() -> None:
    ...
    parser.add_argument(
        "-d",
        "--date",
        help="指定下载日期，格式 YYYYMMDD，如 20260326",
    )
    parser.add_argument(
        "-a",
        "--all",
    ...
    downloader = UDIDownloader("daily", output_format, args.db)
    ...
    if args.date:
        ...
        if not item:
            print(f"[ERROR] 未找到日期 {args.date} 的数据")
            downloader.list_available()
            return
